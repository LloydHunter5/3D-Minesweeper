...
public class Game : MonoBehaviour
{
    ...
    private void createGrid()
    {
        ...
        if (GlobalConstants.doMandelbulb)
        {
            //Destroy cubes that are not in the mandlebulb
            for (int i = 0; i < WIDTH; i++)
            {
                for (int j = 0; j < HEIGHT; j++)
                {
                    for (int k = 0; k < DEPTH; k++)
                    {
                        ...
                        {
                            if (!Mandelbulb.isInMandelbulb(new Vector3Int(i, j, k), WIDTH, HEIGHT, DEPTH, 30, 8))
                            {
    ...
    private void placeMines()
    {
        for(int a = 0; a < MINE_COUNT; a++)
        {
            ...
            if (GlobalConstants.isChallengeMode)
            {
                foreach (Vector3Int adj in cellsToCheckExtended)
                {
    ...
    private void revCell(Vector3Int pos)
    {
        ...
        if (dangerLevel == 0)
        {
            ...
            Destroy(c);
            totalCells--;
            foreach(Vector3Int adj in GlobalConstants.isChallengeMode ? cellsToCheckExtended : cellsToCheck) { 
    ...
    {
        GlobalConstants.isChallengeMode = !GlobalConstants.isChallengeMode;
    }
}
